...
public class EmployeeDao extends BaseDao {
    ...
    //模糊查询
    public List<Employee> selectConditionEmp (Object[] objs, String condition) {
        //select * from employee where name like '%规%'
        String sql = "select * from employee where name like '%"+condition+ "%' limit ?, ?";
        List<Employee> query = super.query(sql, objs, Employee.class);
        return query;
    }
    //模糊查询的总条数
    public int selectAllConditionSize(String condition) {
        String sql = "select count(*) from employee where name like '%"+condition+ "%'";
        int size = super.querySize(sql);
        return size;
    }
